...
func DialTimeout(c context.Context, network, addr string, timeout time.Duration) (*Client, error) {
	// establish a connection
	conn, err := socket.DialTimeout(c, network, addr, timeout)
	if err != nil {
		return nil, err
	}
...
func Dial(c context.Context, network, addr string) (*Client, error) {
	return DialTimeout(c, network, addr, time.Duration(0))
}
...
// SetContext sets the context on this client.
